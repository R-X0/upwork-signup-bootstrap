...
def save_account(status, **extra):
    """Append this account to a DURABLE JSONL ledger (cap/accounts.jsonl).
    run_creds.txt is overwritten every run and loses prior accounts; this is the
    permanent record. Called at creation (login creds saved even if onboarding
    later stalls) and again with final status/phone at the end."""
    rec={"ts":time.strftime("%Y-%m-%dT%H:%M:%S"),"email":EMAIL,"password":PW,
         "first":fn,"last":ln,"status":status}
    rec.update(extra)
    try:
        with open(os.path.join(CAP,"accounts.jsonl"),"a",encoding="utf-8") as fh:
            fh.write(json.dumps(rec)+"\n")
        log(f"ledger += {status} <{EMAIL}>")
    except Exception as e:
        log(f"ledger write err {e}")
    return rec
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        while True: await asyncio.sleep(5)
        # ===== legacy inline onboarding below is now UNREACHABLE (superseded by run_onboarding) =====
        await page.goto("https://www.upwork.com/nx/create-profile/",wait_until="domcontentloaded",timeout=60000)
        await page.wait_for_timeout(4000)
        ...
        for i in range(16):
            ...
            if "please-verify" in url or "signup/verify" in url:
                log(f"bounced to verify page ({url}) — email not verified; stopping onboarding"); break
            if "categories" in url:
                # left column = pick 1 category (plain text links); right column = pick 1-3
                # specialties (checkboxes that appear after a category is chosen); then Next.
                cat=await page.query_selector('text="Customer Service"')
                if cat:
                    try: await cat.click(); await page.wait_for_timeout(1800)
                    except Exception: pass
                picked=0
                for s in await page.query_selector_all("input[type=checkbox]"):
                    try:
                        if await s.is_visible() and not await s.is_checked():
                            await s.check(force=True); picked+=1
                            if picked>=2: break
                    except Exception: pass
                if picked==0:  # specialties may be clickable items, not checkboxes
                    for it in await page.query_selector_all('[role=option], [role=listitem], li'):
                        try:
                            t=(await it.inner_text() or "").strip()
                            if t and await it.is_visible() and len(t)<45 and t!="Customer Service":
                                await it.click(); picked+=1; break
                        except Exception: pass
                await page.wait_for_timeout(800)
                c=await click_text(page,"Next, add your skills","Next")
                log(f"step {i} url=categories picked_specialties={picked} next={c}")
                if c is None: log("categories: Next not clickable (no category/specialty?) — stopping"); break
                continue
            if "languages" in url:
            ...
            # resume-import (and any page without Skip/Next) needs the manual escape hatch.
            c=await click_text(page,"Skip for now","Skip","Next","Continue",
                               "Fill out manually","Do it manually","Continue without resume","Save and continue")
            if c is None:
                # self-document: log every visible control so the next stuck page isn't blind,
                # then try any enabled button whose text looks like an advance action.
                ctrls=await page.evaluate("()=>[...document.querySelectorAll('button,a')]"
                    ".filter(e=>e.offsetParent&&(e.innerText||'').trim()).map(e=>e.innerText.trim()).slice(0,30)")
                log(f"step {i} url={url} NO advance label — controls={ctrls}")
                prim=await page.evaluate("()=>{const b=[...document.querySelectorAll('button')]"
                    ".find(x=>!x.disabled&&x.offsetParent&&(x.innerText||'').trim().length<40&&"
                    "/manual|continue|next|skip for now|submit|save and continue|got it/i.test((x.innerText||'').trim()));"
                    "if(b){b.click();return b.innerText.trim();}return null;}")
                log(f"  primary-fallback clicked={prim}")
                if not prim:
                    await page.wait_for_timeout(2000)
                    if page.url==url: log(f"stuck {url}"); break
            else:
        ...
        # PHONE LAST — only rent a TextVerified number if a phone field is ACTUALLY on
        # the page. If the flow stalled earlier (e.g. an unhandled onboarding step), there
        # is no phone input and renting would burn a paid number for nothing.
        phone_el=await page.query_selector('input[type=tel], input[placeholder*="phone" i]')
        if not phone_el:
            log(f"no phone field on {page.url} — SKIPPING TextVerified rental (flow stalled before the phone step)")
            save_account("incomplete", final_url=page.url)
        else:
            s,h=tv(); vid,number=tv_number(s,h)
            digits=normalize_phone(number)
            log(f"TV vid={vid} number={number}")
            save_account("phone_rented", phone=number, tv_vid=vid, final_url=page.url)
            try:
                await phone_el.click(); await page.keyboard.press("Control+A"); await page.keyboard.press("Delete")
            ...
            save_account("complete", phone=number, tv_vid=vid, final_url=page.url)
        await page.screenshot(path=os.path.join(CAP,"hybrid_done.png"),full_page=True)
